        ...
        area(Processing)
        {
            ...
            action(DownloadPdf)
            {
                ...
                var
                    ...
                    Base64String: Text;
                begin
                    Rec.CalcFields(Rec."File Base64 Content");
                    Rec."File Base64 Content".CreateInStream(InStream);

                    InStream.ReadText(Base64String);

                    if (StrLen(Base64String) mod 4 <> 0) then
                        Error('Base64 string length is invalid.');

                    TempBlob.CreateOutStream(OutStream);
                    convert.FromBase64(Base64String, OutStream);
