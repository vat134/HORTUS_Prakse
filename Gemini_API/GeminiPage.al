page 80008 "Gemini dialogue"
{
    ...
    layout
    {
        area(content)
        {
            group("User Input")
            {
                ...
                {
                    ApplicationArea = All;
                    ToolTip = 'Enter request here.';
                }

                field("Input_Promt_Text"; InputPromtText)
                {
                    ApplicationArea = All;
                    ToolTip = 'Enter a query to work with the source. (Salaries_instruction)';
                }
            }
        }
    }
    ...
    actions
    {
        area(processing)
        {
            ...
            action("Send request with protm")
            {
                Caption = 'Send request with protm';
                ApplicationArea = All;
                Promoted = true;
                PromotedCategory = Process;
                PromotedIsBig = true;
                ...
                var
                    ResponseText: Text[10000];
                begin
                    if InputPromtText = '' then begin
                        ResponseText := 'No request text';
                        Message('Please enter some text.');
                    end else
                        ResponseText := MakeGeminiPromtRequest(InputPromtText);
                    Message(ResponseText);
                end;
            }
        }
    ...
    local procedure MakeGeminiPromtRequest(UserInput: Text[1000]): Text[10000]
    var
        HttpClient: HttpClient;
        HttpRequestMessage: HttpRequestMessage;
        HttpResponseMessage: HttpResponseMessage;
